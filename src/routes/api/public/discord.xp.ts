...
export const Route = createFileRoute("/api/public/discord/xp")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        let query = supabaseAdmin.from("profiles").select("user_id, xp, level");
        ...
        return new Response(JSON.stringify({ success: true, user_id: profile.user_id, xp: newXp, level: newLevel }), {
