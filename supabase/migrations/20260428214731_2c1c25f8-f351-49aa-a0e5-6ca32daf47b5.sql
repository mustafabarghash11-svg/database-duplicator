...
DECLARE
  ...
  v_order_id UUID;
BEGIN
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Must be logged in'; END IF;

  SELECT price_points, stock, is_active INTO v_price, v_stock, v_active
    FROM public.shop_items WHERE id = _item_id FOR UPDATE;

  IF NOT FOUND OR NOT v_active THEN RAISE EXCEPTION 'Item not available'; END IF;
  ...
  IF v_balance < v_price THEN RAISE EXCEPTION 'Not enough points'; END IF;

  UPDATE public.profiles SET points = points - v_price WHERE user_id = v_user_id;

  IF v_stock > 0 THEN UPDATE public.shop_items SET stock = stock - 1 WHERE id = _item_id; END IF;
...
DECLARE
  ...
  v_new_level INTEGER;
BEGIN
  UPDATE public.profiles SET xp = xp + 50, points = points + 10
    WHERE user_id = NEW.user_id RETURNING xp INTO v_new_xp;
...
CREATE POLICY "Anyone can update counter" ON public.visitor_counter FOR UPDATE
...
-- ============= SITE DATA (shared content from /devk panel) =============
CREATE TABLE public.site_data (
  id TEXT PRIMARY KEY DEFAULT 'main',
  data JSONB NOT NULL DEFAULT '{}'::jsonb,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
INSERT INTO public.site_data (id, data) VALUES ('main', '{}'::jsonb);
ALTER TABLE public.site_data ENABLE ROW LEVEL SECURITY;

CREATE POLICY "site_data readable by everyone"
  ON public.site_data FOR SELECT USING (true);
CREATE POLICY "site_data admin/mod insert"
  ON public.site_data FOR INSERT TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'moderator'));
CREATE POLICY "site_data admin/mod update"
